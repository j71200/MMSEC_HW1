...
for blkSizeIdx = 1:blkSizeListLen
	...
	for idx = 1:alphaLenth
		[mPSNR(idx, blkSizeIdx) ber(idx, blkSizeIdx) ] = SSmeasuring( InputImage, alpha(idx), blkSize );
	...
	title(['block size = ', num2str(blkSize), '  capacity = ', num2str(capacityList(blkSizeIdx))]);
	xlabel('alpha');
	ylabel('PSNR');
	plot(alpha, mPSNR(:, blkSizeIdx), 'r');
	psnrImageFileName = ['experiment\SS_sliding_alpha\psnr\a_' num2str(alpha_head) '_' num2str(alpha_tail) '_' num2str(alpha_slot) '_b_' num2str(blkSize) '_psnr.png'];
	...
	berImageFileName = ['experiment\SS_sliding_alpha\ber\a_' num2str(alpha_head) '_' num2str(alpha_tail) '_' num2str(alpha_slot) '_b_' num2str(blkSize) '_ber.png'];
...
multipsnrImageFileName = ['experiment\SS_sliding_alpha\psnr\a_' num2str(alpha_head) '_' num2str(alpha_tail) '_' num2str(alpha_slot) '_multipsnr.png'];
...
multiberImageFileName = ['experiment\SS_sliding_alpha\ber\a_' num2str(alpha_head) '_' num2str(alpha_tail) '_' num2str(alpha_slot) '_multiber.png'];
